...
/**
 * The `SoilCompanionServer` class represents the backend server for the Soil Companion application.
 * It provides a set of endpoints and utilities to handle user sessions, WebSocket subscriptions,
 * file uploads, geographic location contexts, feedback submissions, vocabulary lookups, and more.
 *
 * The class incorporates mechanisms for session management, authentication, WebSocket support,
 * health endpoints for system monitoring, and CORS support for cross-origin resource sharing.
 * Additionally, it includes utilities to sanitize input and format output while interacting with
...
object SoilCompanionServer extends MainRoutes {
  ...
  /**
   * Build a per-session map-event sink that sends a "map_data" WebSocket event for each map
   * generated by MapTools during a query. The sink looks up the live WebSocket connection at
   * call time so it works even if the connection was replaced by a reconnect after the assistant
   * instance was first created.
   */
  private def makeMapEventSink(sessionId: String): String => Unit = { mapDataJson =>
    Option(wsConnections.get(sessionId)).foreach { conn =>
      try {
        conn.send(Ws.Text(upickle.default.write(
          QueryEvent("map_data", Some(mapDataJson))
        )))
      } catch {
        case e: Throwable =>
          logger.warn(s"Failed to send map_data event for session $sessionId: ${e.getMessage}")
      }
    }
  }
          ...
          if sessionExpirationMinutes >= 0 then
            ...
            while (it.hasNext) {
              ...
              if (now - last >= deadlineMillis) {
                ...
                assistants.put(sid, AssistantLive(makeMapEventSink(sid)))
  ...
  def subscribe(sessionId: String): WsHandler = {
    ...
    WsHandler { connection =>
      ...
      if (!assistants.containsKey(sessionId) || assistants.get(sessionId) == null) then
        assistants.put(sessionId, AssistantLive(makeMapEventSink(sessionId)))
