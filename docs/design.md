# Design Document: Soil Companion
...
**Project:** [Soil Companion](https://github.com/soilwise-he/natural-language-querying)
...
- **Farmers and land managers** (particularly in The Netherlands) who want accessible field-level agricultural data such as crop history, soil physical properties, and greenness indices.
...
- [SoilWise Project](https://soilwise-he.eu)
- [SoilWise Repository](https://repository.soilwise-he.eu)
- [ISRIC SoilGrids v2.0](https://rest.isric.org/soilgrids/v2.0)
- [WUR AgroDataCube v2](https://agrodatacube.wur.nl)
- [WUR OpenAgroKPI](https://openagrokpi.wur.nl)
- [LangChain4j](https://docs.langchain4j.dev) — Java/JVM LLM integration framework
- [Cask](https://com-lihaoyi.github.io/cask/) — Scala HTTP micro-framework
- [Apache Solr](https://solr.apache.org) — Search engine powering the SoilWise catalog
...
- **OpenAgroKPI (Netherlands)** — Retrieves field-specific KPIs (greenness, soil physical properties, crop rotation indices) from the WUR OpenAgroKPI API.
...
- **RAG (Retrieval-Augmented Generation)** — Local knowledge documents (PDF, text, Markdown) are embedded on startup and used as supplementary context for answers, retrieved by semantic similarity.
...
| **External API timeout** | 15,000 ms (Solr, SoilGrids, AgroDataCube, OpenAgroKPI, Wikipedia) |
...
1. **Agentic tool calling** — The LLM autonomously decides which of 6 tool integrations to invoke (catalog search, SoilGrids, AgroDataCube, OpenAgroKPI, Wikipedia, vocabulary SPARQL), executing up to 10 sequential tool-call iterations per query.
2. **RAG from local knowledge** — Documents (PDF, text, Markdown) are split into chunks, embedded with a local model (AllMiniLmL6V2), and stored in memory. Relevant chunks are retrieved by cosine similarity and injected into the prompt.
...
| **WUR OpenAgroKPI** | API key header (`OPENAGRO_ACCESS_TOKEN`) | `OPENAGRO_BASE_URL` | NL field-level KPIs (greenness, soil, crop rotation) |
